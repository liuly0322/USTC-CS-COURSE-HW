...
class LogisticRegression:
    ...
    def __init__(self, penalty="l2", gamma=0.0):
        ...
        self.theta = None
        if penalty == 'l1':
            self.penalty_cost = lambda theta: gamma * \
                np.sum(np.absolute(theta))
            self.penalty_learning = lambda theta: gamma * np.sign(theta)
        elif penalty == 'l2':
            self.penalty_cost = lambda theta: gamma / \
                2 * np.sum(np.square(theta))
    ...
    def fit(self, X, y, lr=0.01, tol=1e-7, max_iter=1e7):
        ...
        for _ in range(int(max_iter)):
            learned = lr * ((self.sigmoid(X @ theta.T) - y).T @
                            X / m + self.penalty_learning(theta))
            if np.max(np.absolute(learned)) < tol:
                break
            theta -= learned

            y_pred = self.sigmoid(X @ theta.T)
            cost_value = -np.sum(y.T @ np.log(y_pred) + (1 - y).T @
                                 np.log(1 - y_pred)) / m + self.penalty_cost(theta)
