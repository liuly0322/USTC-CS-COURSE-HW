...
int main() {
  auto module = new Module("SysY code");  // module name 是什么无关紧要
  ...
  // BasicBlock 的名字在生成中无所谓，但是可以方便阅读
  ...
  builder->create_ret(retLoad);
  // 给这么多注释了，但是可能你们还是会弄很多 bug
  // 所以强烈建议配置 AutoComplete，效率会大大提高！
  // 别人配了 AutoComplete，只花 1 小时 coding
  // 你没有配 AutoComplete，找 method 花 5 小时，debug 花 5 小时，肯定哭唧唧！
  // 最后，如果猜不到某个 IR 指令对应的 C++ 的函数，建议把指令翻译成英语然后在 method 列表中搜索一下
  // 最后的最后，这个例子只涉及到了一点基本的指令生成，
  // 对于额外的指令，包括数组，在之后的实验中可能需要大家好好搜索一下思考一下，
  // 还有涉及到的 C++ 语法，可以在 gitlab 上发 issue 提问或者向大家提供指导
